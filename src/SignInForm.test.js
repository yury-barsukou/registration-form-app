...
describe('SignInForm', () => {
  ...
  describe('Form Validation', () => {
    ...
    test('validates password criteria correctly', () => {
      ...
      expect(screen.getByText(/Your password must have at least 8 characters/i).className).toMatch(/invalid/);
  ...
  describe('Form Submission', () => {
    ...
    test('shows error message on invalid email submission', () => {
      fillOutForm({ email: 'invalid' });
      fireEvent.click(screen.getByRole('button', { name: BUTTON_TEXT }));
      expect(screen.queryByText(/Please enter a valid email address/i)).toBeInTheDocument();
    });

    test('shows error message on invalid password submission', () => {
